...
+ 통계분석
  ...
  |` 기술통계 `| 평균이나 합, 표준편차와 같은 표준적인 분석 |<br>
  |` 다변량 분석 `| 두 개 이상의 변수를 동시에 분석하는 기법 |<br>
  |` 회귀분석 `| 변수들이 결과에 미치는 영향 정도를 추정하는 기법 |<br>
...
* 텍스트 마이닝
  ...
  |` 문서요약 `| 문서의 내용을 추출해 요약 |<br>
  |` 문서분류 `| 문서 내용을키워드에 따라 구조화 |<br>
  |` 문서군집 `| 문서를 분석해 동일한 내용의 문서들을 묶음 |<br>
  |` 특성추출 `| 문서 내에서 사용자가 원하는 정보를 도출 |<br>
